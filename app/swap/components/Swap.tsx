...
function Swap() {
		...
		if (tokens.length > 1) {
			const inAssetIndex = tokens.findIndex(
				(token) =>
					...
					inCurrency?.toString().toLowerCase()
			);
			const outAssetIndex = tokens.findIndex(
				(token) =>
					token.id.toLowerCase() ==
					outCurrency?.toString().toLowerCase()
			);
	...
	const exchange = async () => {
		...
		calls.push(
			rxRouter.interface.encodeFunctionData("updateOracleData", [
				updateData,
			])
		);
	...
	const validate = () => {
		if (!isConnected)
			...
			return { valid: false, message: "Approve" };
			...
			<Head>
				<title>
					...
					{tokenFormatter.format(
						inToken?.price?.toNumber() /
							outToken?.price?.toNumber() || 0
					)}{" "}
