        ...
        for(let i in allPoolsClone){
            ...
            for(let k = 0; k < allPoolsClone[i].synths.length; k++) {
                for(let l = 0; l <allPoolsClone[i].synths[k].synthDayData.length; l++) {
                    ...
                    let totalFee = Big(synthDayData.dailyMinted).div(1e18).mul(allPoolsClone[i].synths[k].mintFee).div(10000).mul(prices[allPoolsClone[i].synths[k].token.id]);
                    // add burn fee
                    totalFee = totalFee.plus(Big(synthDayData.dailyBurned).div(1e18).mul(allPoolsClone[i].synths[k].burnFee).div(10000).mul(prices[allPoolsClone[i].synths[k].token.id]));
            ...
            allPoolsClone[i].debtBurnApr = averageDailyBurn
                ...
                .mul(100)
                .toFixed(2);
            allPoolsClone[i].rewardAPY = Big(allPoolsClone[i]?.rewardSpeeds[0])
                .div(1e18)
