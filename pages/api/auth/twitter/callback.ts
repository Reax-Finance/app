...
import { X_ACCOUNT_AGE_MIN, X_ACCOUNT_FOLLOWERS_MIN } from "../../../../src/const";
...
export default async function TwitterFallback(
	...
	try {
		const args = new URLSearchParams({
			...
			redirect_uri:
				process.env.NEXT_PUBLIC_VERCEL_URL + `/callback/twitter`,
		...
		if (
			...
			return res.status(400).send({ message: `Your account must have at least 20 followers to connect. Please try again with a different account.` });
		} else if (
			new Date().getTime() -
				new Date(userResponse.data.data.created_at).getTime() <
			X_ACCOUNT_AGE_MIN
		) {
			return res.status(400).send({ message: `Your account must be at least 3 months old to connect. Please try again with a different account.` });
		}
		...
		let err = error?.response?.data?.error_description || JSON.stringify(error?.response?.data || error);
