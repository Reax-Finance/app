...
function AddLiquidity({ updatedAccount, setUpdatedAccount, tabIndex }: any) {
  const { chain, isConnected, address } = UserAccount();
  ...
  useEffect(() => {
    ...
    const inUsdScaled = Big(Number(inputAmount) || 0)
      ...
      .mul(ONE_ETH);
    let userTotalBalanceUSD = Big(account.userTotalBalanceUSD).add(inUsdScaled);
    ...
    if (Number(outputAmount) > 0) {
      ...
      calls.push(
        rxRouter.interface.encodeFunctionData("mint", [
          Big(outputAmount).mul(Big(10).pow(outToken.decimals)).toFixed(),
          address,
          address,
        ])
      );
  ...
  const getSteps = () => {
    ...
    if (
      ...
      steps.push({
        ...
        execute: () => delegate(liquidityData.debtToken, rxRouter.address),
      });
    }
    return steps;
  };
