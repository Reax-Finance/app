...
function RemoveLiquidity({ updatedAccount, setUpdatedAccount, tabIndex }: any) {
  ...
  const tokens = reserveData
    ? reserveData.vaults
        .filter(
          (vault) =>
            (Big(vault.userBalance.toString()).gt(0) ||
  ...
  useEffect(() => {
    if (tokens.length > 1) {
      const assetIndex = tokens.findIndex(
        (token) =>
          token.id.toLowerCase() == inputCurrency?.toString().toLowerCase()
      );
  ...
  useEffect(() => {
    ...
    let userTotalBalanceUSD = Big(account.userTotalBalanceUSD).sub(inUsdScaled);
    ...
    let userAdjustedBalanceUSD = Big(account.userAdjustedBalanceUSD).sub(
      inUsdScaled.mul(
        Big(reserveData.vaults[inputAssetIndex].config.baseLTV.toString()).div(
          10000
        )
      )
    );
  ...
  const onInputTokenSelected = (e: number) => {
    const selectedTokenId = tokens[e].id;
    const params = new URLSearchParams(searchParams.toString());
    params.set("inputCurrency", selectedTokenId);
  ...
  const getSteps = () => {
    ...
    if (
      ...
      steps.push({
        type: "APPROVAL",
        execute: () => approveLp(liquidityData.lpToken, rxRouter.address!),
        data: {
  ...
  const validate = () => {
    ...
    if (loading) return { valid: false, message: "Loading..." };
      ...
      (Number(outputAmount) || 0) > 0 &&
      Big(outputAmount).mul(ONE_ETH).gt(outToken.balance.toString())
    )
      return {
        valid: false,
