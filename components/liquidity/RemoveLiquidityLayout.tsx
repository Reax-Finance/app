...
export default function RemoveLiquidityLayout({
  ...
  const maxLp = () => {
    let balance = Big(liquidityData?.lpToken?.balance?.toString())
      .div(ONE_ETH)
      .toString();
    let debt = Big(liquidityData?.debtToken?.balance?.toString())
      .div(ONE_ETH)
      .toString();
  ...
  const maxInput = () => {
    ...
    let max = Big(account.userAdjustedBalanceUSD)
      .sub(
        Big(account.userTotalDebtUSD).sub(
          Big(Number(outputAmount) > 0 ? outputAmount : 0).mul(
            outToken.price.toString()
          )
        )
      )
      .mul(10000)
      .div(reserveData.vaults[inputAssetIndex].config.baseLTV.toString())
    ...
    let balance = Big(tokens[inputAssetIndex].balance.toString())
      .div(10 ** tokens[inputAssetIndex].decimals)
      .toString();
    let zero = Big(0);
              ...
              >
                ...
                {isConnected && (
                  <Flex align={"center"} gap={1}>
                    <Text>
                      {Big(liquidityData?.lpToken?.balance?.toString()).gt(
                        liquidityData?.debtToken?.balance?.toString()
                      )
                        ? "Max"
                        : "Balance"}
                    </Text>
              ...
              >
                ...
                {isConnected && (
                  <Flex align={"center"} gap={1}>
                    ...
                    >
                      {" "}
                      {tokenFormatter.format(Number(maxInput()))}
                    </Text>
                  </Flex>
                )}
              </Flex>{" "}
