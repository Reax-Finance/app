...
export default function AddLiquidityLayout({
  ...
  const handleMaxInput = () => {
    let _inputAmount = Big(
      tokens[inputAssetIndex].walletBalance.toString()
    ).div(Big(10).pow(tokens[inputAssetIndex].decimals).toString());
                    ...
                    >
                      ...
                      {tokenFormatter.format(
                        tokens[inputAssetIndex]
                          ? tokens[inputAssetIndex].walletBalance /
                ...
                <Text>
                  {dollarFormatter.format(
                    (Number(outputAmount) || 0) *
                      (outToken?.price?.div(100_000_000).toNumber() ?? 0)
                  )}
                </Text>
                {isConnected && (
                  <Flex
