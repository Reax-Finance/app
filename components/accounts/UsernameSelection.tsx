			...
			<Dark600Box2C flexDir={'column'} justify={'space-between'} flex={1}>
					...
					<>
						...
						>
							...
							{user?.user?.username ? <Flex color={'green.400'} align={'center'} >
								<BsCheck size={'24px'} />
								<Text>Completed</Text>

							</Flex> : <Flex gap={2} align={"center"}>
								<Heading
									fontSize={"lg"}
									color={"secondary.400"}
								>
						...
						{user?.user?.username ? (<Flex mt={4} p={4} pt={2} gap={2} align={'center'}>
					<Text >
						Your username is: 
					</Text>
					<Heading color={'primary.400'} size={"md"}>{user?.user?.username}</Heading>
						...
						<Text mt={4}>{!isValidInput && error}</Text>
					</>
			</Dark600Box2C>
		</>
	);
