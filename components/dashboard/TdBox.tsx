...
export default function TdBox({isFirst = false, children, isNumeric = false, alignBox}: any) {
  return (
    ...
    <Flex color={'whiteAlpha.700'} align={'center'} justify={isNumeric ? 'end' : 'start'} px={2} h='100%' ml={alignBox == 'left' ? 4 : 0} mr={alignBox == 'right' ? 4 : 0} borderBottom={'1px solid'} borderColor='whiteAlpha.200'>
        {children}
    </Flex>
    </Td>
  )
