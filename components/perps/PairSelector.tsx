      ...
      <Box bg={`${colorMode}Bg.400`} h="500px" cursor={"help"}>
        ...
        <Flex flexDir={"column"} shadow={"2xl"} bg={`${colorMode}Bg.200`}>
          {pairs?.map((pair, index) => {
            return (
              <Link href={`/perps/${pair.id}`} key={index}>
                <Divider />

                <Box _hover={{ bg: `${colorMode}Bg.600` }} px={3} my={0}>
                  <Flex
                    paddingY={1}
                  ...
                  >
                    <Flex align={"center"} gap={2}>
                      ...
                      <Image
                        ...
                        alt="pair"
                      />
                      <Heading fontSize={"md"}>{pair.id}</Heading>
                    </Flex>
                    <Box textAlign={"end"}>
                      {/* <Text fontSize={'xs'} mb={-1} color={colorMode == 'dark' ? 'whiteAlpha.600' : 'blackAlpha.600'}>Price</Text> */}
                      <Text>
                        {(
                          Number(pair.synth1.synth.price) /
                          Number(pair.synth2.synth.price)
                        ).toFixed(4)}
                      </Text>
                    </Box>
                  </Flex>
