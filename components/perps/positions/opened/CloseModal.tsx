...
export default function CloseModal({ details }: any) {
  ...
  const close = async () => {
    ...
    calls.push(
      position.interface.encodeFunctionData("closePosition", [
        ...
        Big(outAmount)
          .mul(isMax ? 10 : 1)
          .mul(
            10 ** details?.debts?.[outAssetIndex]?.market?.inputToken?.decimals
          )
          .toFixed(0),
        supplied.address,
      ])
    ...
    if (isMax && details?.debts.length == 1) {
      ...
      for (let i = 0; i < details?.collaterals.length; i++) {
        ...
        calls.push(
          position.interface.encodeFunctionData("transferOut", [
            supplied.address,
            ethers.constants.MaxUint256,
          ])
        );
  ...
  const _setInputAmount = (e: string) => {
    ...
    if (_outAmount.gt(details.debts[outAssetIndex].debt)) {
      setInputAmount(
        Big(details.debts[outAssetIndex].debt)
          .mul(prices[outToken])
          .div(prices[inToken])
          .toString()
      );
  ...
  const setMax = (multiplier: number) => {
    _setInputAmount(
      Big(details?.collaterals?.[selectedIndex]?.collateral)
        .mul(multiplier)
        .toString()
    );
  };
              ...
              >
                <Flex>
                  ...
                  >
                    <Image
                      src={`/icons/${
                        details?.collaterals?.[selectedIndex]?.market
                          ?.inputToken?.symbol ?? ""
                      }.svg`}
                      w={"26"}
                      h={"26"}
              ...
              >
                <Flex>
                  ...
                  >
                    <Image
                      src={`/icons/${
                        details?.debts?.[outAssetIndex]?.market?.inputToken
                          ?.symbol ?? ""
                      }.svg`}
                      w={"26"}
                      h={"26"}
                    ...
                    <Select
                      ...
                      _focusVisible={{ borderColor: "transparent" }}
                      value={outAssetIndex}
                      onChange={(e) => setOutAssetIndex(Number(e.target.value))}
                    >
