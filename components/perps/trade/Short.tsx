  ...
  const setInputAmount = (e: any) => {
	...
	const validate = () => {
		...
		if (tokens[inAssetIndex].id !== ethers.constants.AddressZero && !data) {
			// If not approved
			if(Big(allowances[tokens[inAssetIndex].id]?.[vaults[selectedPosition]?.id] ?? 0).add(Big(approvedAmount).mul(10 ** (tokens[inAssetIndex].decimals ?? 18))).lt(Big(inAmount).mul(10 ** (tokens[inAssetIndex].decimals ?? 18)))) {
				return {
					stage: 1,
	...
	const open = async () => {
		...
		let _leveragedAmount = ethers.utils.parseUnits(Big(inAmount).mul(prices[tokens[inAssetIndex].id]).mul(leverage).div(prices[pairs[pair].token1.id]).toFixed(tokens[inAssetIndex].decimals, 0), 18);
		let deadline_m = 5;
