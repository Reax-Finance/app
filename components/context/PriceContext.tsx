...
function PriceContextProvider({ children }: any) {
  ...
  const { markets: selectedLendingMarket, pools: lendingPools } =
    useLendingData();
  ...
  React.useEffect(() => {
    if (
      ...
      pools.length > 0 &&
      selectedLendingMarket.length > 0
    ) {
  ...
  const updatePrices = async () => {
    ...
    for (let i = 0; i < pools.length; i++) {
      const priceOracle = new ethers.Contract(
        pools[i].oracle,
        getABI("PriceOracle", chainId),
        helper.provider
      );
      for (let j in pools[i].collaterals) {
        let collateral = pools[i].collaterals[j];
        if (
          collateral.feed == ethers.constants.HashZero.toLowerCase() ||
          collateral.feed.startsWith("0x0000000000000000000000")
        ) {
          reqs.push([
            pools[i].oracle,
            priceOracle.interface.encodeFunctionData("getAssetPrice", [
              pools[i].collaterals[j].token.id,
            ]),
          ]);
        } else {
          if (REPLACED_FEEDS[collateral.feed])
            collateral.feed = REPLACED_FEEDS[collateral.feed];
          pythFeeds.push(collateral.feed);
          if (!_feedToAsset[collateral.feed])
            _feedToAsset[collateral.feed] = [];
      ...
      for (let j in pools[i].synths) {
        ...
        if (
          synth.feed == ethers.constants.HashZero.toLowerCase() ||
          synth.feed.startsWith("0x0000000000000000000000")
        ) {
          reqs.push([
            pools[i].oracle,
            priceOracle.interface.encodeFunctionData("getAssetPrice", [
          ...
          if (REPLACED_FEEDS[synth.feed])
            synth.feed = REPLACED_FEEDS[synth.feed];
    ...
    for (let j = 0; j < lendingPools.length; j++) {
      ...
      for (let i = 0; i < markets.length; i++) {
        ...
        if (
          market.feed == ethers.constants.HashZero.toLowerCase() ||
          market.feed.startsWith("0x0000000000000000000000")
        ) {
          reqs.push([
            market.protocol._priceOracle,
            priceOracle.interface.encodeFunctionData("getAssetPrice", [
          ...
          if (REPLACED_FEEDS[market.feed])
            market.feed = REPLACED_FEEDS[market.feed];
    ...
    if (pythFeeds.length > 0) {
      allReqs.push(pythPriceService.getLatestPriceFeeds(pythFeeds));
    }

    Promise.all(allReqs)
      .then(async ([res, pythRes]: any) => {
        let pythIndex = 0;
        let reqCount = 0;
        for (let i = 0; i < pools.length; i++) {
          for (let j in pools[i].collaterals) {
            if (
              pools[i].collaterals[j].feed ==
                ethers.constants.HashZero.toLowerCase() ||
              pools[i].collaterals[j].feed.startsWith(
                "0x0000000000000000000000"
              )
            ) {
              // update price from feed
              _prices[pools[i].collaterals[j].token.id] = Big(
                BigNumber.from(res.returnData[reqCount]).toString()
              )
                .div(1e8)
                .toString();
              reqCount += 1;
              ...
              _prices[pools[i].collaterals[j].token.id] = Big(
                pythRes[pythIndex].price.price
              )
                .mul(10 ** pythRes[pythIndex].price.expo)
                .toString();
              pythIndex += 1;
            }
          }
          ...
          for (let j in pools[i].synths) {
            if (
              pools[i].synths[j].feed ==
                ethers.constants.HashZero.toLowerCase() ||
              pools[i].synths[j].feed.startsWith("0x0000000000000000000000")
            ) {
              // update price from feed
              _prices[pools[i].synths[j].token.id] = Big(
                BigNumber.from(res.returnData[reqCount]).toString()
              )
                .div(1e8)
                .toString();
        ...
        for (let j = 0; j < lendingPools.length; j++) {
          ...
          for (let i = 0; i < markets.length; i++) {
            ...
            if (
              ...
              _prices[market.inputToken.id] = Big(
                BigNumber.from(res.returnData[reqCount]).toString()
              )
                .div(1e8)
                .toString();
        ...
        if (pythFeeds.length > 0) {
          await pythPriceService.subscribePriceFeedUpdates(
            pythFeeds,
            (feed: any) => updatePythPrices(feed, _feedToAsset, _prices)
          );
        }
  ...
  const updatePythPrices = (res: any, feedToAsset: any, _prices: any) => {
    for (let i in feedToAsset["0x" + res.id]) {
      _prices[feedToAsset["0x" + res.id][i]] = Big(res.price.price)
        .mul(10 ** res.price.expo)
        .toString();
